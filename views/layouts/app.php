...
<div id="app" class="flex h-full">
  <aside id="sidebar" class="w-64 bg-gray-900 text-white flex flex-col shrink-0 transition-all">
    <!-- Logo -->
    <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-700">
      <div class="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center font-bold text-sm">W</div>
      <span class="font-semibold text-lg tracking-tight">Workday</span>
    </div>

    <!-- Nav principal -->
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
      <a href="<?= APP_URL ?>/dashboard" class="sidebar-link <?= str_contains($_SERVER['REQUEST_URI'],'/dashboard')?'active':'' ?>">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
        Dashboard
      </a>
      <a href="<?= APP_URL ?>/boards" class="sidebar-link <?= str_contains($_SERVER['REQUEST_URI'],'/boards')?'active':'' ?>">
    ...
    <div class="border-t border-gray-700 p-4">
      <div class="flex items-center gap-2 mb-2">
        <a href="<?= APP_URL ?>/reports" class="sidebar-link text-xs w-full <?= str_contains($_SERVER['REQUEST_URI'],'/reports')?'active':'' ?>">
          📊 Relatórios
        </a>
      </div>
  ...
  <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
    ...
    <header class="h-14 bg-white border-b border-gray-200 flex items-center px-6 gap-4 shrink-0">
      <button id="sidebarToggle" class="text-gray-500 hover:text-gray-900 lg:hidden">
